...
namespace Priceless
{
	public partial class ListaCompra : ContentPage
	{
		...
		public async void LimpaLista(object sender, EventArgs e)
		{
			bool resposta = await DisplayAlert("Deseja mesmo apagar a lista ?", "Atenção", "Sim", "Não");
			if (resposta)
			{
				((App)App.Current).produtoCompraViewModel.DeleteAll<ProdutoCompra>();
				((App)App.Current).produtoCompraViewModel.AtualizaLista();
				((App)App.Current).produtoCompraViewModel.Total = ((App)App.Current).produtoCompraViewModel.TotalCompra();
			}
		}
